...
Set of tools to generate (or aid in generating) stochastic turbulence with the Mann or TurbSim method. You first create a stochasticTurbulence object, and it can have either Mann or TurbSim data associated with it. It can also have FAST output data as "fastOut".

Some of these are io-related and should be moved to the io repository.
...
from miscellaneous import parse, smooth_spectrum
from meteorology import power_law
...
class stochasticTurbulence:
    ...
    def __init__(self,prefix='prefix',D=154.0):
        ...
        D : float,
            some rotor diameter which comes in handy when trying to determine points above/below the rotor, etc.
        """
        self.prefix = prefix         
        self.D = D
        self.R = D/2.0
    ...
    def initialize_mannBox(self, seed=1209, flagHF=True, gamma=0,
        ...
        ----------        
        seed : int,
            used when calling the Mann command line turbulence generator
        flagHF : bool,
            another parameter used when calling the Mann command line turbulence generator
        gamma : float,
        ...
        target_TI : float,
            target hub height turbulence intensity [-]
        target_uHub : float,
            target hub height mean wind speed [m/s]
        zHub : float,
            hub height [m]
        T : float,
            total length of time desired [s]
        """         
    ...
    def generate_mannBox(self,pathToExecutable,outPath=None, \
        ...
        pathToExecutable : string,
            where to find executable of Mann command line tool, which can be downloaded from http://www.hawc2.dk/download/pre-processing-tools -- only works if running from a windows OS
        outPath : string,
            absolute path to where you want to work (i.e. write out your .bat file and generate your turbulence .bin files)
        justCreateBat : bool,
            if you're running this from a mac, you likely just want to create your .bat file and go manually run it on a windows OS to generate your turbulence .bin files
        overwriteDir : bool,
            whether to overwrite the outDir
        """
        if outPath is None:
    ...
    def readBTS(self,pathToBinary):
        ...
        ----------
        pathToBinary : string,
            where to find binary (a single <prefix>.bts) output        
        """      
    ...
    def visualize(self,component='U',time=0):
        """
        Quick peek at the data for a given component, at a specific time.

        Parameters
        ----------
        component : str,
            whether to plot "U", "u", "v", "w"
        time : int,
